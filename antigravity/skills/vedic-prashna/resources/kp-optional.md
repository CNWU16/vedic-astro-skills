...
Timing 只在 promise 为 `positive_only`、temporary-retrograde gate 清空且 operative
...
`mixed` 必须写成正反宫同时命中，不能压缩成含糊的“可能”；失败关闭状态不能被
模型补全。“先说人话”一节不得出现 cusp、sub-lord、RP、period、宫位号、机械
状态码或生产状态；这些全部放进可跳过的技术附录。机械状态码、英文缩写和生产
状态不得代替开头的人话回答。进入完成条件：判读单每一项都能回查
`structured_kp.json`。
